...
import com.ng.util.AutoObjectFactory;
...
public class BrowserEmulator {
	 ...
	 * @param map_url
	 */
	public void clickElement(String node, String map_url) {
		pause(stepInterval);
		factory = new AutoObjectFactory();
		By by = factory.getElementBy(node, map_url);
		expectElementExistOrNot(true, by, timeout);

		try {
			clickTheClickableElement(by, System.currentTimeMillis(), 2500);
		} catch (Exception e) {
			e.printStackTrace();
			handleFailure("Failed to click " + by.toString());
		}
	 ...
	 * @return
	 */
	public By getClickElementBy(String node, String map_url) {
		pause(stepInterval);
		factory = new AutoObjectFactory();
		By by = factory.getElementBy(node, map_url);
		expectElementExistOrNot(true, by, timeout);
	...
	private void clickTheClickableElement(By by, long startTime, int timeout)
			throws Exception {
		try {
			browserCore.findElement(by).click();
		} catch (Exception e) {
			if (System.currentTimeMillis() - startTime > timeout) {
				logger.info("Element " + by.toString() + " is unclickable");
				throw new Exception(e);
			} else {
				Thread.sleep(500);
				logger.info("Element " + by.toString()
						+ " is unclickable, try again");
				clickTheClickableElement(by, startTime, timeout);
			}
		}
	}
	...
	public void typeElement(String node, String map_url, String text) {
		pause(stepInterval);
		factory = new AutoObjectFactory();
		By by = factory.getElementBy(node, map_url);
		expectElementExistOrNot(true, by, timeout);

		WebElement we = browserCore.findElement(by);
		try {
			we.clear();
		} catch (Exception e) {
			logger.warn("Failed to clear text at " + by.toString());
		}
		try {
			we.sendKeys(text);
		} catch (Exception e) {
			e.printStackTrace();
			handleFailure("Failed to type " + text + " at " + by.toString());
		}

		logger.info("Type " + text + " at " + by.toString());
	}
	 ...
	 * @param timeout
	 */
	public void expectElementExistOrNot(boolean expectExist, final By by,
			int timeout) {
		if (expectExist) {
			try {
				new Wait() {
					public boolean until() {
						return isElementPresent(by, -1);
					}
				}.wait("Failed to find element " + by.toString(), timeout);
			} catch (Exception e) {
				e.printStackTrace();
				handleFailure("Failed to find element " + by.toString());
			}
			logger.info("Found desired element " + by.toString());
		} else {
			if (isElementPresent(by, timeout)) {
				handleFailure("Found undesired element " + by.toString());
			} else {
				logger.info("Not found undesired element " + by.toString());
			}
		}
	}
